...
import { submitContact, submitInscription, submitDoleance, submitBourse } from '@/lib/forms'
...
export function AdhesionPage({ onBack }: { onBack: () => void }) {
  ...
  return (
    <PageShell
      ...
      sub="Choisissez la formule qui correspond à votre activité et rejoignez des milliers d'actifs de l'informel en Afrique."
      color="#FF2D7A"
      heroImage={imgH1}
      onBack={onBack}
      heroChildren={
...
export function DoleancesPage({ onBack }: { onBack: () => void }) {
  ...
  const submitDoleanceForm = async () => {
    if (!type || !nom.trim() || !contact.trim() || !description.trim()) {
      setErr("Veuillez remplir le type, votre nom, un contact et la description.")
      return
    }
    setErr("")
    setSending(true)
    const res = await submitDoleance({ type, nom, contact, espace, description })
    setSending(false)
    if (!res.ok) { setErr("Enregistrement impossible pour le moment. Réessayez ou contactez-nous via WhatsApp."); return }
    setSent(true)
  }
        ...
        <div className="rounded-3xl p-5 bg-white" style={{ border: '1px solid #F3E8D8', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          ...
          {!sent ? (
            <div className="space-y-4">
              ...
              {[
                { label: "Nom et Prenom", ph: "Votre nom complet", val: nom, on: setNom },
                { label: "Telephone / Email", ph: "+229 xx xx xx xx ou email@exemple.com", val: contact, on: setContact },
                { label: "Espace concerne", ph: "Ex: ASSURANCE, MARKET, KAASH...", val: espace, on: setEspace },
              ].map(f => (
                <div key={f.label}>
                  <label className="block text-[#374151] text-[12px] font-bold mb-1.5">{f.label}</label>
                  <input value={f.val} onChange={e => f.on(e.target.value)} placeholder={f.ph}
              ...
              <div>
                ...
                <textarea rows={4} value={description} onChange={e => setDescription(e.target.value)} placeholder="Decrivez precisement votre probleme ou suggestion..."
              ...
              <button
                onClick={submitDoleanceForm} disabled={sending}
                ...
                {sending ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> Enregistrement...</> : <>Envoyer ma doléance <ArrowRight size={16} /></>}
              </button>
            </div>
          ) : (
...
export function MentionsLegalesPage({ onBack }: { onBack: () => void }) {
  const SECTIONS = [
    ...
    },
    {
      ...
      content: "IPPOO est l'entité N°1 accompagnant les actifs de l'informel en Afrique : une plateforme intégrée regroupant 21 espaces thématiques interconnectés couvrant les domaines de la finance, du commerce, de la santé, de l'emploi, de l'agriculture, des services et du bien-être.\n\nServices concrets, impact durable : l'objectif est d'accompagner les acteurs de l'informel africain dans leur développement économique, leur accès aux marchés et leur protection sociale.",
    },
    {
...
export function AProposPage({ onBack }: { onBack: () => void }) {
  ...
  return (
    <PageShell title="À Propos d'IPPOO" sub="Découvrez la mission, les valeurs et les ambitions de l'entité N°1 accompagnant les actifs de l'informel en Afrique." color="#FF2D7A" heroImage={imgH10} onBack={onBack}
      heroChildren={
        <div>
          <div className="grid grid-cols-3 gap-2 mb-5">
    ...
    >
      ...
      <div className="px-4 py-6">
        ...
        <p className="text-[#374151] text-[13px] leading-relaxed mb-3">
          <IpHL text="IPPOO est une marque propriété de APTDC-Z-UP · TDO · LIMITED, organisation africaine au service des actifs de l'informel, fondée à Parakou, au Borgou, République du Bénin." />
        </p>
      ...
      <div className="px-4 py-6">
        ...
        <p className="text-[#6B7280] text-[12px] mb-4">21 espaces thématiques couvrant tous les secteurs de l'informel</p>
        <div className="flex flex-wrap gap-2">
      ...
      <div className="px-4 py-6">
        ...
        <p className="text-[#374151] text-[13px] leading-relaxed mb-4">
          <IpHL text="IPPOO intervient dans l'ensemble des secteurs de l'informel en Afrique, depuis Parakou, au Bénin, avec une ambition continentale." />
        </p>
        <div className="flex flex-wrap gap-2">
          {[
...
export function BourseValeurPage({ onBack }: { onBack: () => void }) {
  ...
  const [sent, setSent] = useState(false)
  const [sending, setSending] = useState(false)
  const [bourseErr, setBourseErr] = useState("")
      ...
      <div id="bourse-form" className="px-4 py-8" style={{ background: 'white' }}>
        ...
        {sent ? (
          ...
          <div className="space-y-3">
            ...
            {bourseErr && (
              <div className="flex items-center gap-2 p-3 rounded-xl text-[13px] text-[#991B1B]" style={{ background: '#FEF2F2', border: '1px solid #FECACA' }}>
                <AlertCircle size={14} className="shrink-0" /><span>{bourseErr}</span>
              </div>
            )}
            <button
              disabled={sending}
              onClick={async () => {
                if (!formData.nom || !formData.tel || !formData.consent) { setBourseErr("Renseignez au moins votre nom, votre téléphone et acceptez d'être recontacté."); return }
                setBourseErr(""); setSending(true)
                const res = await submitBourse(formData)
                setSending(false)
                if (!res.ok) { setBourseErr("Enregistrement impossible pour le moment. Réessayez ou contactez-nous via WhatsApp."); return }
                setSent(true)
              }}
              className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl text-[15px] font-bold text-white mt-2 transition-opacity disabled:opacity-60"
              style={{ background: `linear-gradient(135deg, ${GOLD}, #B8860B)`, boxShadow: `0 8px 24px ${GOLD}44`, opacity: formData.consent ? 1 : 0.6 }}>
              {sending ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> Enregistrement...</> : <><ArrowRight size={16} /> Envoyer ma demande</>}
            </button>
          </div>
        )}
      </div>
...
/* ══════════════════════════════════════════════════════════════
   PAGE CONTACT — WhatsApp / e-mail (sans clé API)
══════════════════════════════════════════════════════════════ */
/* Canaux de contact IPPOO — sans clé API, sans backend exposé.
   Les soumissions de formulaires sont routées vers WhatsApp (canal principal)
   avec un repli e-mail (mailto) pré-rempli. */
const IPPOO_WHATSAPP = '2290141521092'
const IPPOO_EMAIL = 'ippooz.up.2@gmail.com'

function openWhatsApp(message: string) {
  const url = `https://wa.me/${IPPOO_WHATSAPP}?text=${encodeURIComponent(message)}`
...
export function ContactPage({ onBack }: { onBack: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setErrMsg(res.error || 'Enregistrement impossible.'); setStatus('error'); return }
      const recap = [
        '*Nouveau message — IPPOO*',
        `Nom : ${form.nom}`,
        `Email : ${form.email}`,
        ...
        <div className="rounded-3xl p-5 bg-white" style={{ border: '1px solid #F3E8D8', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          ...
          {status === 'success' ? (
            <div className="text-center py-8">
              ...
              <h3 className="font-black text-[20px] text-[#111827] mb-2">Message prêt à envoyer !</h3>
              <p className="text-[#6B7280] text-[14px] leading-relaxed mb-5">
                WhatsApp s'est ouvert avec votre message pré-rempli — appuyez sur <strong>Envoyer</strong> pour finaliser. Notre équipe vous recontactera sous 24h.
              </p>
              <div className="flex flex-col gap-2.5">
                <a
                  href={`mailto:${IPPOO_EMAIL}?subject=${encodeURIComponent(`[IPPOO Contact] ${form.sujet || 'Nouvelle demande'}`)}&body=${encodeURIComponent(`Nom : ${form.nom}\nEmail : ${form.email}\nTéléphone : ${form.telephone || '—'}\n\n${form.message}`)}`}
                  className="px-6 py-3 rounded-2xl text-[13px] font-bold text-[#374151]"
...
/* ══════════════════════════════════════════════════════════════
   PAGE INSCRIPTION — WhatsApp récapitulatif (sans clé API)
══════════════════════════════════════════════════════════════ */
export function InscriptionPage({ onBack }: { onBack: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrMsg('')
    try {
      const res = await submitInscription(form)
      if (!res.ok) { setErrMsg(res.error || 'Enregistrement impossible.'); setStatus('error'); return }
      const offreLabel = form.offre === 'base' ? 'Offre de Base — 750 FCFA/jour' : 'Couverture Complémentaire'
      const recap = [
        '*Nouvelle inscription — IPPOO*',
  ...
  return (
    <PageShell
      title="Rejoindre IPPOO"
      sub="Inscrivez-vous à l'écosystème IPPOO et accédez à 21 espaces thématiques dédiés aux actifs de l'informel en Afrique..."
      color="#FF2D7A"
      heroImage={imgH1}
      onBack={onBack}
      heroChildren={
      ...
      <div className="px-4 py-6">
        ...
        {status === 'success' ? (
          <div className="rounded-3xl p-6 bg-white text-center" style={{ border: '1px solid #BBF7D0', boxShadow: '0 8px 32px rgba(22,163,74,0.12)' }}>
            ...
            <h3 className="font-black text-[22px] text-[#111827] mb-2">Presque terminé, {form.prenom} !</h3>
            <p className="text-[#6B7280] text-[14px] leading-relaxed mb-2">
              Bienvenue dans l'écosystème <strong className="text-[#FF2D7A]">IPPOO</strong> !
            </p>
            <p className="text-[#6B7280] text-[14px] leading-relaxed mb-6">
              WhatsApp s'est ouvert avec votre récapitulatif — appuyez sur <strong>Envoyer</strong> pour valider votre inscription. Notre équipe vous contactera sous 24h pour finaliser votre adhésion.
            </p>
            <div className="grid grid-cols-2 gap-3">
