...
import { FichesSuiviPage, ParametresPage } from '@/screens/MemberPages'
import { AdminLoginPage } from '@/screens/AdminLoginPage'
import { AdminDashboard } from '@/screens/AdminDashboard'
import { useAuth0 } from '@auth0/auth0-react'
import { supabase } from '@/lib/supabase'

type PageKey = 'home' | 'adhesion' | 'parrainage' | 'business' | 'faq' | 'actualites' | 'doleances' | 'mentions' | 'privacy' | 'cgu' | 'apropos' | 'programmes' | 'sante' | 'groupements' | 'investissement' | 'emploi' | 'moncompte' | 'souscriptions' | 'kaash' | 'bourse' | 'contact' | 'inscription' | 'login' | 'register' | 'onboarding' | 'app' | 'fiches' | 'parametres' | 'admin' | 'adminlogin'

/* Clés de pages valides pour le routage par chemin (/page) — URLs propres,
 indexables par les moteurs de recherche. Le retour navigateur et le
 rafraîchissement restaurent la bonne page (pas l'accueil). Le fallback SPA
 (Cloudflare not_found_handling: single-page-application) sert index.html pour
 les accès directs à /page. */
const PAGE_KEYS: PageKey[] = ['home','adhesion','parrainage','business','faq','actualites','doleances','mentions','privacy','cgu','apropos','programmes','sante','groupements','investissement','emploi','moncompte','souscriptions','kaash','bourse','contact','inscription','login','register','onboarding','app','fiches','parametres','admin','adminlogin']
/* Préfixe de base Vite : '/' en production, éventuel sous-chemin en aperçu Figma. */
const BASE = (import.meta.env.BASE_URL || '/').replace(/\/$/, '')
/* Construit l'URL d'une page : BASE pour l'accueil, BASE/page sinon. */
const pageToPath = (p: PageKey): string => (p === 'home' ? BASE + '/' : BASE + '/' + p)
const pageFromPath = (): PageKey => {
 if (typeof window === 'undefined') return 'home'
 let path = window.location.pathname
 if (BASE && path.startsWith(BASE)) path = path.slice(BASE.length)
 const key = decodeURIComponent(path.replace(/^\/+|\/+$/g, '')).trim()
 return (PAGE_KEYS as string[]).includes(key) ? (key as PageKey) : 'home'
}
...
const SLIDES = [
 ...
 title: ['L\'entité N°1', "accompagnant les actifs", "de l'informel en Afrique."],
 sub: "IPPOO, l'entité N°1 accompagnant les actifs de l'informel en Afrique : plateforme intégrée, services concrets, impact durable. 21 espaces thématiques pour financer, connecter et faire grandir chaque acteur.",
...
export default function App() {
 ...
 const [currentPage, setCurrentPage] = useState<PageKey>(pageFromPath)
 const [toasts, setToasts] = useState<Toast[]>([])
 const [emergencyOpen, setEmergencyOpen] = useState(false)
 const [formSent, setFormSent] = useState(false)
 const [adminEmail, setAdminEmail] = useState<string | null>(() => {
  try { const raw = sessionStorage.getItem('ippoo_admin_session'); return raw ? JSON.parse(raw).email : null } catch { return null }
 })

 const adminLogin = (email: string, password: string): boolean => {
  const ok = email === (import.meta.env.VITE_ADMIN_EMAIL || '') && password === (import.meta.env.VITE_ADMIN_PASSWORD || '')
  if (ok) { sessionStorage.setItem('ippoo_admin_session', JSON.stringify({ email, loggedAt: Date.now() })); setAdminEmail(email) }
  return ok
 }
 const adminLogout = () => { sessionStorage.removeItem('ippoo_admin_session'); setAdminEmail(null); goPage('home') }

 /* ── Membre (Auth0) : onboarding + espace application ────────── */
 const { isAuthenticated, isLoading: authLoading, user, logout: auth0Logout } = useAuth0()
 const memberLogout = () => auth0Logout({ logoutParams: { returnTo: window.location.origin } })

 /* Après retour d'Auth0 (inscription/connexion), on oriente le membre :
  profil incomplet → onboarding, sinon → son espace application. Le drapeau
  `ippoo_post_auth` est posé par les écrans Connexion/Inscription avant la
  redirection, pour ne pas détourner la navigation d'un simple visiteur connecté. */
 useEffect(() => {
  if (authLoading || !isAuthenticated || !user?.sub) return
  const pending = sessionStorage.getItem('ippoo_post_auth')
  if (!pending) return
  let cancelled = false
  supabase
   .from('profiles')
   ...
   .then(({ data }) => {
    if (cancelled) return
    sessionStorage.removeItem('ippoo_post_auth')
    goPage(data ? 'app' : 'onboarding')
   })
 ...
 window.history.pushState({ page: p, scrollY: 0 }, '', pageToPath(p))
 ...
 window.history.replaceState({ page: initial, scrollY: 0 }, '', pageToPath(initial))
 window.scrollTo({ top: 0, behavior: 'instant' })
 ...
 if (currentPage === 'login') return <LoginPage onBack={goBack} onGoRegister={() => goPage('register')} />
 if (currentPage === 'register') return <RegisterPage onBack={goBack} onGoLogin={() => goPage('login')} />
 if (currentPage === 'onboarding') return <OnboardingPage onComplete={() => goPage('app')} />
 if (currentPage === 'app') {
  if (authLoading) return <div className="min-h-screen grid place-items-center" style={{ background:'#FFF8F2' }}><div className="w-8 h-8 rounded-full border-2 border-[#FF2D7A] border-t-transparent animate-spin" /></div>
  if (!isAuthenticated) return <LoginPage onBack={goHome} onGoRegister={() => goPage('register')} />
  return <MemberDashboard platforms={PLATFORMS} onNavigate={p => goPage(p as PageKey)} onLogout={memberLogout} onHome={goHome} />
 }
 if (currentPage === 'fiches') {
  if (authLoading) return <div className="min-h-screen grid place-items-center" style={{ background:'#FFF8F2' }}><div className="w-8 h-8 rounded-full border-2 border-[#FF2D7A] border-t-transparent animate-spin" /></div>
  if (!isAuthenticated) return <LoginPage onBack={goHome} onGoRegister={() => goPage('register')} />
  return <FichesSuiviPage onBack={() => goPage('app')} onNavigate={p => goPage(p as PageKey)} />
 }
 if (currentPage === 'parametres') {
  if (authLoading) return <div className="min-h-screen grid place-items-center" style={{ background:'#FFF8F2' }}><div className="w-8 h-8 rounded-full border-2 border-[#FF2D7A] border-t-transparent animate-spin" /></div>
  if (!isAuthenticated) return <LoginPage onBack={goHome} onGoRegister={() => goPage('register')} />
  return <ParametresPage onBack={() => goPage('app')} onNavigate={p => goPage(p as PageKey)} onLogout={memberLogout} />
 }
 if (currentPage === 'adminlogin' || (currentPage === 'admin' && !adminEmail))
  return <AdminLoginPage onLogin={adminLogin} onBack={goBack} />
 if (currentPage === 'admin' && adminEmail)
  return <AdminDashboard adminEmail={adminEmail} onLogout={adminLogout} />
 return null
 })()
 ...
 return (
 <button key={tab.id} onClick={() => { if (tab.id === "moncompte") goPage(isAuthenticated ? "app" : "login"); else if (tab.page) goPage(tab.page); else goHome() }}
 ...
 <IpHL text="IPPOO valorise le savoir-faire, la créativité et le talent des actifs de l'informel en Afrique. Mode, artisanat, gastronomie, culture : chaque création trouve sa plateforme." />
 </p>
 <a href="#espaces"
 ...
 {<IpHL text="Étudiants, diaspora, producteurs, artisans, novateurs : chaque actif de l'informel a sa place dans l'écosystème IPPOO. Positionnement clair, services adaptés et accompagnement sur mesure." />}
 </p>
 ...
 const isActive = tab.page ? currentPage === tab.page : false
 const handleClick = () => {
 if (tab.id === 'moncompte') { goPage(isAuthenticated ? 'app' : 'login'); return }
